import { Bell, CheckCircle2, ChefHat, Clock, Home, Receipt, RotateCcw, Sparkles } from 'lucide-react';
...
const STATUS_CONFIG: Record<OrderStatus, { title: string; subtitle: string; icon: React.ElementType }> = {
    pending: { title: 'Order Received', subtitle: 'Waiting for confirmation...', icon: Clock },
    confirmed: { title: 'Cooking Soon', subtitle: 'Kitchen received your order', icon: CheckCircle2 },
    preparing: { title: 'Preparing', subtitle: 'Chefs are working their magic', icon: ChefHat },
    ready: { title: 'Ready for Pickup', subtitle: 'Grab it at the counter!', icon: Bell },
    completed: { title: 'Enjoy!', subtitle: 'Have a wonderful meal', icon: Sparkles },
    cancelled: { title: 'Cancelled', subtitle: 'This order was cancelled', icon: Clock },
};
...
export function OrderTrackingScreen({
    ...
    const currentStatus = useOrderSubscription(orderId || '', status);

    // Use the realtime status if available, otherwise fallback to prop
    const displayStatus = currentStatus || status;

    const steps = getTrackingSteps(displayStatus, createdAt);
    ...
    const isActive = displayStatus === 'confirmed' || displayStatus === 'preparing';
    ...
    return (
        <div className="min-h-screen bg-slate-50/50 pb-[120px] font-sans selection:bg-indigo-100 animate-fade-in">
            {/* Header - Glass */}
                ...
                <div className="px-5 py-3 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs font-medium text-slate-400">
                            {tableCode ? `Table ${tableCode} • ` : ''}{createdAt.toLocaleDateString()}
                        </p>
                    </div>
                </div>
            </header>
            ...
            {/* Bottom Actions - Floating Glass Island */}
