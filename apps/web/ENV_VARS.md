...
| `GOOGLE_GEMINI_API_KEY` | Gemini API Key | YES (Edge) | Used for AI categorization/images |
| `NEXT_PUBLIC_APP_URL` | App Base URL | YES | Used for metadata/redirects |

## Notes
- Do NOT expose `SUPABASE_SERVICE_ROLE_KEY` or `GOOGLE_GEMINI_API_KEY` with `NEXT_PUBLIC_`.
