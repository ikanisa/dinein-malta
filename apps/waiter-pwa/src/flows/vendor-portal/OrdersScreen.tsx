...
export function OrdersScreen() {
    ...
    return (
        <div className="min-h-screen bg-white pb-28">
            {/* Header */}
            <div className="sticky top-0 z-30 bg-white border-b border-slate-100 px-6 py-4">
                        ...
                        {TABS.map(tab => (
                            ...
                            >
                                ...
                                {tab.id === 'new' && newCount > 0 && (
                                    <span className="ml-1.5 text-rose-600 font-bold">{newCount}</span>
                                )}
                            </button>
                        ))}
                    </div>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-mono"
                    />
                </div>
            </div>
                    ...
                    <div className="border border-slate-100 rounded-lg divide-y divide-slate-50">
                        {filteredOrders.map((order, idx) => {
                            ...
                            return (
                                <div
                                    key={order.id}
                                    className="p-4 bg-white hover:bg-slate-50 transition-colors animate-fade-in group"
                                    style={{ animationDelay: `${idx * 30}ms` }}
                                >
                                    <div className="flex items-start justify-between">
                                        {/* Left Side */}
                                        <div className="flex gap-4">
                                            <div className="w-16 flex flex-col items-center justify-center bg-slate-50 rounded border border-slate-200 p-1.5 h-14">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="font-mono text-sm font-bold text-slate-900">#{order.order_code}</span>
                                                    <span className={`flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${config.bg} ${config.text}`}>
                                                ...
                                                <div className="flex items-center gap-3 text-xs text-slate-400">
                                                    ...
                                                    {order.notes && (
                                                        <span className="text-amber-600 font-medium px-1.5 py-0.5 bg-amber-50 rounded">
                                                            Note included
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        <div className="flex flex-col items-end gap-2">
                                            <span className="font-mono font-bold text-slate-900">${order.total_amount.toFixed(2)}</span>
                                            {config.action && config.nextStatus && (
                                                <button
                                                    onClick={() => updateOrderStatus(order.id, config.nextStatus!)}
                                                    className="px-3 py-1.5 bg-white border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 text-slate-700 text-xs font-bold rounded shadow-sm transition-all active:scale-95"
                                                >
                                                    {config.action}
                                                </button>
                                            )}
